...
/* Active bead glow + pulse */
@keyframes beadPulse {
    0% { transform: scale(1); box-shadow: 0 0 6px rgba(255, 193, 7, 0.6); }
    50% { transform: scale(1.08); box-shadow: 0 0 14px rgba(255, 193, 7, 0.9); }
    100% { transform: scale(1); box-shadow: 0 0 6px rgba(255, 193, 7, 0.6); }
}

.active-bead {
    animation: beadPulse 1.2s ease-in-out 2;
    background: radial-gradient(circle at 30% 30%, #ffe9a8 0%, #ffd54f 45%, #ffca28 80%) !important;
    border: 1px solid #ffb300 !important;
}

/* Previous bead dimming (D1 subtle) */
.prev-bead {
...
if "daily_goal" not in st.session_state:
    st.session_state.daily_goal = 16
...
def update_streak():
    ...
    if st.session_state.history.get(today, 0) >= goal:
        if st.session_state.history.get(yesterday, 0) >= goal:
...
with st.container():
    ...
    with col_left:
        ...
        prev_index = max(active_index - 1, -1)
        ...
        for i in range(BEADS_PER_ROUND):
            ...
            # Base Tulasi bead with subtle grain
            base_style = (
                "radial-gradient(circle at 30% 30%, #f7e7cf 0%, #d9bc96 45%, #b89263 80%)"
            )

            # Active bead glow
            if i == active_index:
                extra_class = "active-bead"
            # Previous bead dimming
            elif i == prev_index:
                extra_class = "prev-bead"
            else:
                extra_class = ""
